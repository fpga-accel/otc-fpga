...
if [ "x$fis_info" == "x" ];then
    echo -e "ERROR:fis tool is not detected, please refer to otc-fpga/README.md 2.1.2, go to intall fisclient."
    exit
fi
fischeck_info=`which fischeck 2> /dev/null`
if [ "x$fischeck_info" == "x" ];then
    echo -e "ERROR:fischeck tool is not detected, please refer to otc-fpga/README.md 2.1.2, go to intall fisclient."
    exit
fi
...
if [ "$mode" == "OCL" ];then
	...
	if [ -f sdk_aeiid.xclbin ];then
        rm -fr ./sdk_aeiid.xclbin
    fi
	...
	/software/Xilinx/SDx_2017.4_op/SDx/2017.4.op/runtime/bin/xclbincat -s METADATA split-xclbin.xml -s MEM_TOPOLOGY split-mem_topology.bin -s CONNECTIVITY split-connectivity.bin -s IP_LAYOUT split-ip_layout.bin -s BINARY_HEADER header.bin -o sdk_aeiid.xclbin
    rm -fr ./*.bin
    rm -fr ./*.xml
    rm -fr ./*.rtd
	cd $script_path
    ...
    if [ -f "$script_path/../prj/bin/manifest.json" ];then
        ...
        rm -fr ./*.dcp
        rm -fr ./manifest.json
		rm -fr $script_path/../prj/bin/*.dcp
		rm -fr $script_path/../prj/bin/manifest.json
    else
